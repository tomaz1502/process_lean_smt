...
theorem th0 :
  ...
  let let10 := (Not let5)
  let let11 := (Not let3)
(Implies grn let5) → (Implies org let3) → (Implies rd1 let1) → (Eq grnx5fMR grn) → (Eq orgx5fMR org) → (Eq grnx5fSR rd1) → (Eq orgx5fSR rd2) → (Eq redx5fSR (Or grn org)) → (Eq redx5fSR (Or grnx5fMR orgx5fMR)) → circuit → rd2 → redx5fMR → (Or grnx5fSR orgx5fSR) → (Not let7) → False :=
  let let1 := (Not rd2)
  let let2 := (Not rd1)
  let let3 := (And let2 let1)
  let let4 := (Not org)
...
fun lean_a13 : (Not let7) => by
have lean_s0 : (Or let7 (Or org let6)) := by timed cnfEquivNeg1
have lean_s1 : (Or let4 let3) := by timed impliesElim lean_a1
have lean_s2 : (Or let11 let1) := by timed @cnfAndPos [let2, let1] 1
have lean_s3 : let11 := by R2 lean_s2, lean_a10, rd2, [(- 1), 0]
...
have lean_s7 : (Or let9 let5) := by timed impliesElim lean_a0
have lean_s8 : (Or let10 let1) := by timed @cnfAndPos [let4, let2, let1] 2
have lean_s9 : let10 := by R2 lean_s8, lean_a10, rd2, [(- 1), 0]
