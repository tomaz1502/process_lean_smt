...
let4 → (Eq let5 let1) → (Eq let7 let6) → (Eq let8 i1) → (Eq let11 let10) → (Eq let9 let2) → (Eq let12 False) → (Eq mode cir) → i1 → let6 → let13 → let14 → let16 → (Not (Eq r1 b1)) → (Not (Eq i2 (Not let5))) → False :=
  let let1 := (Eq b2 r2)
  let let2 := (Not let1)
  let let3 := (Not let2)
  let let4 := (Eq i2 let3)
...
fun lean_r3 : (Eq let8 i1) => -- THEORY_REWRITE_BOOL
...
fun lean_a14 : (Not (Eq i2 (Not let5))) => by
...
have lean_s5 : (Eq i1 let8) := by timed Eq.symm lean_r3
