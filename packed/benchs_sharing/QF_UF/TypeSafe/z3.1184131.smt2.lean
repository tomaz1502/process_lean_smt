...
(Eq let3 True) → (Eq let4 False) → (Not (Eq f1 f2)) → (Not (Eq let2 f1)) → let6 → let7 → let7 → let6 → False :=
  let let1 := (f8 f9)
  let let2 := (f3 f4 f5 f6 f7 let1)
  let let3 := (Eq let2 let2)
  let let4 := (Not True)
...
have lean_s1 : let11 := by timed Eq.symm lean_a7
have lean_s2 : (And let7 let11) := by timed And.intro lean_a6 lean_s1
