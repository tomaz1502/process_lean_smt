...
theorem th0 :
  ...
  let let22 := (Not let7)
  let let23 := (Not let17)
(Eq let7 let6) → (Eq let4 let10) → (Eq let10 let11) → (Eq let9 let8) → (Eq let13 let12) → (Eq let14 let2) → (Eq let17 let16) → (Eq let3 let18) → (Eq let20 let19) → (Eq let21 let12) → (Not (Eq f1 f2)) → let22 → let23 → let23 → False :=
  let let1 := (Int.ofNat 1)
  let let2 := (binrel% HMul.hMul (Int.ofNat 4) f3)
  let let3 := (binrel% HAdd.hAdd let2 let1)
  let let4 := (binrel% HSub.hSub let3 let1)
...
have lean_s23 : let19 := by timed eqResolve lean_a13 lean_s22
