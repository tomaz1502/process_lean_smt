...
theorem th0 :
  ...
  let let13 := (Not let10)
  let let14 := (Not let7)
(Eq let7 let6) → (Eq let3 let8) → (Eq let11 let10) → (Eq let12 let9) → (Eq let13 let9) → (Not (Eq f1 f2)) → let14 → let12 → False :=
  let let1 := (Int.ofNat 1)
  let let2 := (binrel% HMul.hMul (Int.ofNat 4) f3)
  let let3 := (binrel% HAdd.hAdd let2 let1)
  let let4 := (Int.ofNat 2)
...
fun lean_a6 : let14 =>
fun lean_a7 : let12 => by
have lean_s0 : let9 := by timed eqResolve lean_a7 lean_r3
