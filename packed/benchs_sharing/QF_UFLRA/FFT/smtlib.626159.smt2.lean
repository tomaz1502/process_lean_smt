...
theorem th0 :
  ...
  let let18 := (Not let5)
(Eq let5 let4) → (Eq let1 let7) → (Eq let6 let9) → (Eq let12 let10) → (Eq let14 let10) → (Eq let17 let16) → (Eq let15 let10) → (Not (Eq f1 f2)) → let18 → let17 → (binrel% LT.lt f3 (binrel% HMul.hMul (Rat.ofInt 2) f4)) → False :=
  let let1 := (binrel% HSub.hSub f3 f4)
  let let2 := (Rat.ofInt 0)
  let let3 := (binrel% GE.ge let2 let1)
  let let4 := (Not let3)
...
have lean_s15 : let13 := by timed eqResolve lean_a9 lean_s14
